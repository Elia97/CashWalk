...
const CustomTooltip = ({ active, payload, label, currency }: TrendTooltipProps) => {
        ...
        {payload.map((item) => (
          <li key={item.dataKey} className="flex items-center gap-2">
            ...
            <span className="font-medium">
              {item.name}: {formatCurrency((item.value as number) ?? 0, currency)}
            </span>
          </li>
        ))}
      </ul>
...
export function MonthlyTrend({
  ...
  return (
    <ResponsiveContainer width="100%" height={320}>
      <BarChart data={data} margin={{ top: 24, right: 16, left: 16, bottom: 8 }} barGap={2}>
        <CartesianGrid stroke="rgba(148, 163, 184, 0.35)" strokeDasharray="0" vertical={false} />
        <XAxis
          dataKey="month"
          tickLine={false}
